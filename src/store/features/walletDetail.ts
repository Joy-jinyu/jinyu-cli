...
export const { updateList, getInitState, updatePage, updateInfo } = walletDetailSlice.actions;
// 修改table
...
export const asyncGetPageList = (id = '') => (dispatch: any, getState) => {
    ...
    return request.post({ url: '/transactions/queryByPage', query: { address: nfrIds, pageStart, pageSize } })
        .then(res => {
...
export const asyncGetNfrDetail = (id = '') => async (dispatch: any, getState) => {
    ...
    const info = await request.post({ url: '/dashboard/search', query: { address: walletId } });
